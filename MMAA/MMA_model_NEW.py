...
class MMAA(torch.nn.Module):
    ...
    def forward(self):
        ...
        for m in range(self.numModalities):
            ...
            if self.loss_robust:
                mle_loss_m = -self.T[m] / 2 * (torch.log(torch.tensor(2 * torch.pi)) + torch.log(torch.sum(loss_per_sub)/self.T[m] + self.epsilon)) - torch.tensor(loss_per_sub/(2 * loss_per_sub/self.T[m] +1))
                mle_loss += mle_loss_m
