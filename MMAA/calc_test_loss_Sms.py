    ...
    for modalityComb in modalityCombs:
        ...
        if os.path.exists(path): 
            ...
            for split in [0,1]:
                ...
                # define train- and testset depending on split. timesteps for each modality defined as well
                X_train = Real_Data(subjects=arguments.get("subjects"), split = (0 if split == 0 else 1))
                X_test = Real_Data(subjects=arguments.get("subjects"), split = (1 if split == 0 else 0))
                ...
                for numArcheTypes in range(2,20+1,2):
                    ...
                    for seed in seeds:
                        ...
                        for idx, modality in enumerate(modalityComb): 
                            ...
                            # S (m, s, k, V) is unique for each subject and modality
                    ...
                    for key, loss_list in modalities_loss.items():
                        # find the mean, std and minimum loss for each modality across seeds
                        mean = np.mean(loss_list)
                        std = np.std(loss_list)
                        min_loss = np.min(loss_list)
                        
                        # save the mean, std and min loss for each modality'
